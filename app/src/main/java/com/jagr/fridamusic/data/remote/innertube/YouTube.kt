...
object YouTube {
    ...
    suspend fun getTranscript(videoId: String): String? = withContext(Dispatchers.IO) {
        try {
            ...
            val subtitles = streamInfo.subtitles ?: return@withContext null
            if (subtitles.isEmpty()) return@withContext null

            val preferredFormatOrder = listOf(
                org.schabi.newpipe.extractor.MediaFormat.TRANSCRIPT3,
                org.schabi.newpipe.extractor.MediaFormat.VTT
            )
            val track = preferredFormatOrder.firstNotNullOfOrNull { format ->
                subtitles.filter { it.format == format }
                    .sortedBy { it.isAutoGenerated }
                    .firstOrNull()
            } ?: subtitles.first()

            val url = track.getUrl() ?: track.getContent()
            if (url.isNullOrBlank()) return@withContext null

            fetchTextBody(url)
        } catch (e: Exception) {
            Log.w("YouTubeSearch", "getTranscript failed for $videoId: ${e.message}")
            null
        }
    }

    private fun fetchTextBody(url: String): String? {
        return try {
            ...
            Log.w("YouTubeSearch", "Failed to download transcript body: ${e.message}")
            null
        }
    }
}
