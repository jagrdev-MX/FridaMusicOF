...
object YouTube {
    ...
    suspend fun getTranscript(videoId: String): String? {
        ...
        for (baseUrl in PIPED_INSTANCES) {
            try {
                val response: JsonObject = client.get("$baseUrl/streams/$videoId") {
                    header("User-Agent", "FridaMusic/1.0")
                    header("Accept-Language", "en-US,en;q=0.9,es;q=0.8")
                }.body()

                val subtitle = response["subtitles"]?.jsonArray
                    ?.mapNotNull { it.jsonObject }
                    ?.sortedWith(
                        compareBy<JsonObject> {
                            val code = it["code"]?.jsonPrimitive?.contentOrNull.orEmpty()
                            when {
                                code.startsWith("en", ignoreCase = true) -> 0
                                code.startsWith("es", ignoreCase = true) -> 1
                                else -> 2
                            }
                        }.thenBy {
                            it["autoGenerated"]?.jsonPrimitive?.booleanOrNull ?: true
                        }
                    )
                    ?.firstOrNull()

                val subtitleUrl = subtitle?.get("url")?.jsonPrimitive?.contentOrNull?.takeIf { it.isNotBlank() }
                    ?: continue
                val transcript = client.get(subtitleUrl) {
                    header("User-Agent", "FridaMusic/1.0")
                }.body<String>()
